...
const ModalAddAnggaran = ({ getListAnggaran }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res?.status === true) {
        await swal.success('Anggaran Berhasil Disimpan!')
        getListAnggaran()
        dispatch(setToggleModal({ isOpen: false, modal: '' }))
        setNominalAnggaran('')
        setKeteranganAnggaran('')
      } else {
        await swal.error('Anggaran Gagal Disimpan!')
      }
    } catch (error) {
                ...
                <label className="block">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border border-slate-200 bg-white py-3 pl-11 pr-4 text-slate-700 shadow-sm outline-none transition focus:border-blue-400"
                    />
                  </div>
                </label>
...
export default ModalAddAnggaran