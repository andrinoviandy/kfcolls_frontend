...
// COMPONENT
// =========================
const TableUserManagement = ({ dimensionScreenW, check, loginAccess }) => {
  ...
  const headerTable = [
    ...
    { label: "NIP", icon: <FaIdCard /> },
    { label: "Nama", icon: <FaUser /> },
    { label: "Username", icon: <FaUserShield /> },
    { label: "Cabang Aktif", icon: <FaBuilding /> },
    { label: "Unit / Role Aktif", icon: <FaUserShield /> },
    { label: "Sub Unit", icon: <FaUniversity /> },
    { label: "Jabatan Aktif", icon: <FaBriefcase /> },
    { label: "Tanggal Aktif", icon: <FaCalendarAlt /> },
    { label: "Email", icon: <FaEnvelope /> },
    { label: "Status", icon: <FaInfoCircle /> },
  ];

  const getListUserManagement = async () => {
    try {
      setAllData([])
      setLoading(true);

      const response = await storeSchema.actions.getListUserManagement({
        page: currentPage,
        limit: perPage,
        keyword: keyword,
        sortBy: sortBy,
                    ...
                    {allData.length === 0 ? (
                        ...
                        {allData.map((v, i) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 whitespace-nowrap flex flex-col gap-2">
                              ...
                              {
                              // Number(v?.cabang_id) === '2000' && !['RL00', 'RL01'].includes(v?.role_id)
                              v?.jenis_user_id && v?.tipe_user === '2' && (
                                <span
                                  className={`
                                    ...
                                    shadow-sm
                                    ${v?.jenis_user_id !== '1' ? 'bg-yellow-700' : 'bg-green-300'}
                            ...
                            {/* TGL AKTIF */}
                            <td className="px-4 py-3 whitespace-nowrap">
                              {formatDate(v?.tgl_aktif_bekerja) || '-'}
                            </td>
                      ...
                      `}
                              >
                                ...
                                {v.flag_aktif === 'Y' ? 'Aktif' : 'Non Aktif'}
