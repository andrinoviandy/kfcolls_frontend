...
const ModalCard1 = () => {
  ...
  useEffect(() => {
    if (toggleModal?.isOpen && toggleModal?.modal === "modalCard1") {
      if (toggleModal?.tipe_card === 'total_pengajuan') {
        setTitleCard('Total Pengajuan')
        setTitleIcon(<FaFileAlt className='text-blue-900' />)
      }

      if (toggleModal?.tipe_card === 'pengajuan_baru') {
        setTitleCard('Pengajuan Baru')
        setTitleIcon(<FaPlusCircle className='text-blue-600' />)
      }

      if (toggleModal?.tipe_card === 'menunggu_verifikasi') {
        setTitleCard('Menunggu Verifikasi')
        setTitleIcon(<FaHourglassHalf className='text-yellow-600' />)
      }

      // MENUNGGU PEMBAYARAN
      if (toggleModal?.tipe_card === 'menunggu_pembayaran') {
        setTitleCard('Menunggu Pembayaran')
        setTitleIcon(<FaMoneyCheckAlt className='text-cyan-600' />)
      }

      if (toggleModal?.tipe_card === 'sudah_dibayarkan') {
        setTitleCard('Sudah Dibayarkan')
        setTitleIcon(<FaMoneyBillWave className='text-green-500' />)
      }

      if (toggleModal?.tipe_card === 'ditolak') {
        setTitleCard('Ditolak')
        setTitleIcon(<FaTimes className='text-red-500' />)
      }
      getListAllPengajuan()
      setCurrentPage(1);
    }
