...
import ModalAddAnggaran from "./Modal/ModalAddAnggaran";
import ModalTransferAnggaran from "./Modal/ModalTransferAnggaran";
...
const TableManajemenAnggaran = ({ loginAccess, dispatch, setToggleModal, toggleModal, dimensionScreenW, check, reloadData, setReloadData }) => {
  ...
  const headerTable = [
    ...
    { label: "Cabang", icon: <FaBuilding /> },
    { label: "GL Account", icon: <FaTags /> },
    { label: "Account Desc", icon: <FaMoneyBillWave /> },
    { label: "Bulan", icon: <FaCalendarAlt /> },
    { label: "Budget Biaya", icon: <FaWallet /> },
    { label: "Realisasi Biaya", icon: <FaMoneyBillWave /> },
    { label: "Sisa Anggaran", icon: <FaWallet /> },
    // { label: "Pengajuan", icon: <FaClipboardList /> },
    { label: "", icon: <FaPercent /> },
  ...
  const getListAnggaran = async () => {
    try {
      ...
      const response = await storeSchema.actions.getListAnggaran({
        page: currentPage,
        limit: perPage,
        keyword: keyword,
        sortBy: sortBy,
  ...
  const handleRiwayat = async (anggaran_id) => {
    try {
      swal.loading()
      const res = await storeSchema.actions.getDetailAnggaran(anggaran_id)
      if (res?.status === true) {
        swal.close();
        dispatch(setToggleModal({ isOpen: !toggleModal.isOpen, modal: "modalRiwayatAnggaran", data: res?.data }));
      } else {
  ...
  const handleAddAnggaran = async (anggaran_id) => {
    try {
      swal.loading()
      const res = await storeSchema.actions.getDetailAnggaran(anggaran_id)
      if (res?.status === true) {
        swal.close();
        dispatch(setToggleModal({ isOpen: !toggleModal.isOpen, modal: "modalAddAnggaran", data: res?.data }));
      } else {
        swal.error('Gagal Mendapatkan Data !')
      }
    } catch (error) {
  ...
  const handleTransferAnggaran = async (anggaran_id) => {
    try {
      swal.loading()
      const res = await storeSchema.actions.getDetailAnggaran(anggaran_id)
      if (res?.status === true) {
        swal.close();
        dispatch(setToggleModal({ isOpen: !toggleModal.isOpen, modal: "modalTransferAnggaran", data: res?.data }));
      } else {
        swal.error('Gagal Mendapatkan Data !')
      }
    } catch (error) {
  ...
  return (
    <>
      <ModalAddAnggaran getListAnggaran={getListAnggaran} />
      <ModalTransferAnggaran getListAnggaran={getListAnggaran} />
                  ...
                  <tbody className="bg-white">
                    ...
                    {tableData?.map((v, i) => (
                      ...
                      >
                            ...
                            >
                              ...
                              {['RL08', 'RL00'].includes(loginAccess?.role_id) && (
                                <>
                                  ...
                                  <ul>
                                    <li>
                                      <div className="pl-0" onClick={() => handleAddAnggaran(v?.anggaran_id)}>
                                  ...
                                  <ul>
                                    <li>
                                      <div className="pl-0" onClick={() => handleTransferAnggaran(v?.anggaran_id)}>
                              ...
                              {['RL08', 'RL00', 'RL06'].includes(loginAccess?.role_id) && (
                                <>
                                  <hr className="my-2" />

                                  <ul>
                                    <li>
                                      <div className="pl-0" onClick={() => handleRiwayat(v?.anggaran_id)}>
                        ...
                        {/* ANGGARAN */}
                        <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-semibold text-blue-800">
                          {formatRupiah(v.budget_biaya)}
                        </td>

                        {/* ANGGARAN TERPAKAI */}
                        <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-semibold text-orange-600">
                          {formatRupiah(v.realisasi_biaya)}
                        </td>

                        {/* SISA ANGGARAN */}
                        <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-semibold text-green-700">
                          {formatRupiah(v?.sisa_anggaran)}
                        </td>

                        {/* SISA ANGGARAN */}
                        {/* <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-semibold text-yellow-700">
                          {formatRupiah(v?.pengajuan_biaya)}
                        </td> */}

                        <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-semibold text-green-700">
                          {((v?.realisasi_biaya ?? 0) / (v?.budget_biaya ?? 0)) ? ((v?.realisasi_biaya ?? 0) / (v?.budget_biaya ?? 0) * 100).toFixed(2) + '%' : '-'}
                        </td>
