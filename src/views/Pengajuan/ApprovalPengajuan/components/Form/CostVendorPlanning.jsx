...
const CostVendorPlanning = ({ data, dataCostPersonilPlanning, getDetailProject, getCostPersonilPlanning, dataCBB }) => {
  const headerTable = ['Vendor', 'Role', 'Kualifikasi', 'Qty', 'UoM', 'Qty', 'UoM', 'Unit Cost', 'Cost', ''];
  ...
  useEffect(() => {
    ...
    //         }
    //       })
    if (data?.DOKUMEN_VENDOR && data?.DOKUMEN_VENDOR.length > 0) {
      let dataZ = []
      data?.DOKUMEN_VENDOR.map((item) => {
        dataZ.push(
          {
            label: item?.UR_VENDOR,
  ...
  useEffect(() => {
    setTotalCost(0);
    const valueCostPersonilPlanning = dataCostPersonilPlanning?.PERSONEL_VENDOR;
    if (valueCostPersonilPlanning?.length > 0) {
      const newData = valueCostPersonilPlanning?.map((value) => {
        ...
        return {
          project_vendor_id: value?.PROJECT_ID,
...
console.log('data', dataFields);
  ...
  const uploadCostPersonilPlanning = async (e, i, divisi_id) => {
    ...
    try {
      ...
      const payload = {
        project_id: value?.project_vendor_id,
        position_id: value?.position_id,
        kualifikasi_id: value?.kualifikasi_id,
        divisi_id: divisi_id,
        qty_person: value?.qty_person,
...
export default CostVendorPlanning