...
const ActionButton = ({ location, navigation, loginAccess, isApprovalPengajuan, setDropdownOpen, dispatch, v, iframeLoading, setIframeLoading, getListPengajuan }) => {
  const pdfRef = useRef();
    ...
    navigation("/edit-pengajuan", {
      state: {
        ...
        data: {
          ...
          ...(v?.child?.length > 0 && v?.jenis_biaya_id === 'KP11' ? { child_id: v?.child[0]?.pengajuan_id } : {})
        }
      },
    });
  };
  ...
  const drawHeader = (doc, qrDataUrl) => {
    ...
    // AREA TENGAH HEADER
    // ==========================
    const leftArea = logoX + logoWidth + 8;
    const rightArea = qrX - 8;
    const centerX = (leftArea + rightArea) / 2;
    ...
    doc.setFontSize(10);
    doc.text(
      "Jl. Budi Utomo No.1, Jakarta Pusat, Indonesia 10710",
      centerX,
      21,
      { align: "center" }
    );
    ...
    <>
      <div className="dropdown dropdown-right" onClick={() => { handleDropdownClick() }}>
        ...
        >
          ...
          {v?.history_id && !["RL01"].includes(loginAccess?.role_id) && ['S1', 'S2', 'T'].includes(v?.user_kd_status) && (
            <>
              <hr className="my-2" />
              <ul>
                <li>
          ...
          {["RL01"].includes(loginAccess?.role_id) && ['KP05', 'KP06', 'KP07'].includes(v?.jenis_biaya_id) && ['S1', 'S2'].includes(v?.kd_status) && v?.child?.length === 0 && !isApprovalPengajuan && v?.user_view_only === 'T' && (
            <>
              <hr className="my-2" />
              ...
              <ul>
                <li>
                  <div
                    className="pl-0"
                    onClick={handlePenyelesaianKasbon}
                  >
          ...
          {loginAccess?.role_id !== "RL01" &&
            isApprovalPengajuan && v?.user_view_only === 'T' && (
              <>
                {v?.user_flag_action === 'Y' ? (
                  <>
                    {(loginAccess?.verifikator === 'Y' && v?.user_status_verifikasi !== 'Y' && v?.user_jenis_user_id === loginAccess?.jenis_user_id) ? (
                      <>
                        <hr className="my-2" />
                    ...
                    ) : loginAccess?.verifikator === 'Y' && (<>
                      {/* <hr className="my-2" />Anda Sudah Melakukan Verifikasi Pada Data Pengajuan Ini */}
                    </>)}
                  ...
                  {/* <hr className="my-2" />Anda Sudah Melakukan Approval Pada Data Pengajuan Ini */}
                </>}
              </>
            )}
          {/* {v?.STATUS === 'Selesai' && ( */}
