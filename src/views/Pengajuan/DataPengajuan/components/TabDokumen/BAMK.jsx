...
const BAMK = ({ location, data, getDetailProject, isVendorRealization, isBillingRealization, view }) => {
  ...
  const handleUpload = async (e, i) => {
    ...
    try {
      ...
      if (!value.no_dokumen.trim()) {
        swal.custom("Input Required", "Nomor BAMK harus diisi!", "warning");
        return;
      }

      if (!value.tgl_dokumen.trim()) {
        swal.custom("Input Required", "Tanggal BAMK harus diisi!", "warning");
        return;
      }
                ...
                {dataFields && dataFields.length > 0 && dataFields?.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className='flex gap-3'>
                      ...
                      {((isVendorRealization === false || isBillingRealization) || data?.FLAG_EDIT && view === true) && (
