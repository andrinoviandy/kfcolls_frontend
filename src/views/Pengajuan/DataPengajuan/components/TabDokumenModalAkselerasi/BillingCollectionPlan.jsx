...
const BillingCollectionPlan = ({ data, getDetailProject, isDetailModalAkselerasi, isBillingRealization }) => {
  ...
  const [headerTable, setHeaderTable] = useState(['Termin', 'Keterangan', 'Divisi', 'Nominal Estimasi Billing']);
  ...
  useEffect(() => {
    const getOptDivisi = async () => {
      try {
        const res = await storeSchema.actions.getReferensiByJenis("div_billing_id");
        if (res?.status === true) {
  ...
  return (
    <>
      ...
      >
            ...
            {dataFieldsDocDelivery?.map((item, index) => (
              <tr key={index}>
                ...
                <td className='flex gap-3'>
                  ...
                  {((isDetailModalAkselerasi !== true && dataFields[toggleModal?.index]?.flag_edit === 1 && !isBillingRealization)) && (
        ...
        <div type="button" className='btn btn-sm rounded-[25px] border-none bg-blue-50 text-primary my-3 w-60 ' onClick={handleAddFieldDoc} disabled={((dataFieldsDocDelivery.filter(a => a.status.canUpload === true).length) === 1 || dataFields[toggleModal?.index]?.flag_edit === 0 || isBillingRealization) ? true : false}>
                    ...
                    {isBillingRealization && (
                      <>
                        ...
                        <td className='flex gap-3 w-full'>
                          ...
                          {(item?.flag_edit === 1 || !isBillingRealization) && (
                            <div className='flex items-center gap-4'>
                              ...
                              {(item?.status?.canUpload || isBillingRealization) && (
                                <>
                                  <div className="tooltip" data-tip="Submit To Finance">
