...
// COMPONENT
// =========================
const TableMasterData = ({ dimensionScreenW, check, loginAccess }) => {
  ...
  const headerTable = [
    ...
    { label: "Kode Referensi", icon: <FaDatabase /> },
    { label: "Nama Referensi", icon: <FaTag /> },
    { label: "Uraian Referensi", icon: <FaInfoCircle /> },
    { label: "Keterangan", icon: <FaNoteSticky /> },
    { label: "Tanggal Create", icon: <FaCalendarAlt /> },
    { label: "Created By", icon: <FaUser /> },
    { label: "Status", icon: <FaInfoCircle /> },
  ];

  const getListMasterData = async () => {
    try {
      setAllData([])
      setLoading(true);

      const response = await storeSchema.actions.getListMasterData({
        page: currentPage,
        limit: perPage,
        keyword: keyword,
        sortBy: sortBy,
